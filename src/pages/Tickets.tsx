...
export default function Tickets() {
  ...
  return (
    <div className="h-full flex bg-background">
      {/* Queue rail */}
      <div className="hidden lg:flex w-52 shrink-0 flex-col border-r border-border bg-surface/40 p-3 gap-0.5">
        <div className="px-2 pt-1 pb-2 text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Queues</div>
        {queues.map(q => {
          const Icon = q.icon;
        ...
        <div className="px-2 pt-5 pb-2 text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Tags</div>
        {["urgent","vpn","sso","onboarding","security"].map(t => (
          <button key={t} className="flex items-center gap-2 px-2.5 py-1.5 rounded-md text-xs hover:bg-surface-2 text-foreground/70">
            <Hash className="h-3 w-3" /> {t}
          </button>
        ))}
        <div className="mt-auto pt-3 px-2 text-[10px] text-muted-foreground/70 leading-relaxed">
          <div className="flex items-center gap-1.5 mb-1.5"><Zap className="h-3 w-3" /> Shortcuts</div>
          <div className="space-y-0.5">
            <div><Kbd>j</Kbd> <Kbd>k</Kbd> next / prev</div>
            <div><Kbd>r</Kbd> reply &nbsp;<Kbd>/</Kbd> search</div>
            <div><Kbd>⌘</Kbd> <Kbd>↵</Kbd> send</div>
          </div>
        </div>
      </div>
