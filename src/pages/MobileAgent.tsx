...
type Filter = "open" | "mine" | "urgent" | "today";
...
export default function MobileAgent() {
  ...
  const counts = useMemo(() => ({
    ...
    mine: tickets.filter((t) => t.assignee_email === myEmail && t.status !== "resolved" && t.status !== "closed").length,
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "mine") list = list.filter((t) => t.assignee_email === myEmail && t.status !== "resolved");
    if (filter === "urgent") list = list.filter((t) => t.priority === "urgent" && t.status !== "resolved");
    if (filter === "today") list = list.filter((t) => new Date(t.created_at ?? 0).toDateString() === new Date().toDateString());
    if (q.trim()) {
      const Q = q.toLowerCase();
      list = list.filter((t) => t.subject?.toLowerCase().includes(Q) || t.ticket_number?.toLowerCase().includes(Q));
    }
    return list.slice(0, 50);
  }, [tickets, filter, myEmail, q]);
  ...
  return (
    <div className="md:hidden min-h-screen bg-background pb-20">
      ...
      <header className="sticky top-0 z-30 bg-background/95 backdrop-blur-xl border-b">
        ...
        <Tabs value={filter} onValueChange={(v) => setFilter(v as Filter)} className="px-2">
            ...
            <SwipeableRow
              ...
              left={{
                ...
                onAction: () => {
                  ...
                  sendPush("Ticket resolved", `${t.ticket_number} — ${t.subject}`);
                },
              }}
              right={{
                label: "Assign me",
                ...
                color: "bg-primary",
                onAction: () => {
                  updateTicket(t.id, { assignee_email: myEmail });
                  sendPush("Assigned to you", `${t.ticket_number} — ${t.subject}`);
                },
              }}
            >
              <button onClick={() => nav(`/app/tickets?id=${t.id}`)} className="w-full text-left px-4 py-3 active:bg-accent/40">
                <div className="flex items-start gap-3">
                  <div className={cn("mt-1 h-2 w-2 rounded-full shrink-0", t.priority === "urgent" ? "bg-rose-500" : t.priority === "high" ? "bg-amber-500" : "bg-muted-foreground/40")} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono text-muted-foreground">{t.ticket_number}</span>
                      <Badge className={cn("text-[10px] px-1.5 py-0 h-4", STATUS_COLOR[t.status] ?? "")}>{t.status}</Badge>
                      <Badge className={cn("text-[10px] px-1.5 py-0 h-4 capitalize", PRIORITY_COLOR[t.priority] ?? "")}>{t.priority}</Badge>
                      <span className="ml-auto text-[10px] text-muted-foreground">{timeAgo(t.updated_at ?? t.created_at)}</span>
                    </div>
                    <div className="text-sm font-medium line-clamp-2 mt-1">{t.subject}</div>
                    <div className="text-[11px] text-muted-foreground mt-0.5 truncate">
                      {t.requester_email} · {t.assignee_email ? `→ ${t.assignee_email.split("@")[0]}` : "Unassigned"}
                    </div>
                  </div>
